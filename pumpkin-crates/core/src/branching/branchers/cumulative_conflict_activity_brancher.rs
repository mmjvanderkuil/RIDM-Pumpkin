use num::abs;
use pumpkin_checking::{AtomicConstraint, CheckerVariable};
...
#[derive(Debug)]
pub struct CpVsids<BackupBrancher> {
    ...
    heap: KeyValueHeap<(DomainId, PredicateType), f64>,
    interval_tracker: HashMap<(DomainId, PredicateType), HashMap<i32,f64>>,
...
impl DefaultBrancher {
    ...
    pub fn default_over_all_variables(assignments: &Assignments) -> DefaultBrancher {
        CpVsids {
            ...
            interval_tracker: Default::default(),
            should_synchronise: false,
            statistics: CpVsidsStatistics::default(),
            max_eq: None,
            max_ne: None,
        }
    }
...
create_statistics_struct!(CpVsidsStatistics {
    ...
    vsids_cca_same_decision: usize,
    point_decisions: usize,
    average_difference_vsids_cca_value: CumulativeMovingAverage<usize>,
    cca_decisions: usize,
...
const EXTREME_PERCENTILE: f64 = 0.90;

impl StorageKey for (DomainId, PredicateType) {
    fn index(&self) -> usize {
        self.0.index() * 4 + match self.1 {
            PredicateType::UpperBound => 0,
            PredicateType::LowerBound => 1,
            PredicateType::Equal => 2,
            PredicateType::NotEqual => 3,
        }
    }
    fn create_from_index(index: usize) -> Self {
        let domain = DomainId::create_from_index (index / 4usize);
        let remainder = index % 4;
        match remainder {
            0 => (domain, PredicateType::UpperBound),
            1 => (domain, PredicateType::LowerBound),
            2 => (domain, PredicateType::Equal),
            3 => (domain, PredicateType::NotEqual),
            4_usize.. => unreachable!(),
        }
    }
}
...
impl StorageKey for i32 {
    ...
    fn create_from_index(index: usize) -> Self {
        match index % 2 {
            0 => - ((index / 2) as i32),
            1 => ((index + 1) / 2) as i32,
            2_usize.. => unreachable!("usize modulo 2 is never 2 or higher. Current list is exhaustive")
        }
    }
}
...
impl<BackupSelector> CpVsids<BackupSelector> {
    /// Creates a new instance with default values for
    /// the parameters (`1.0` for the increment, `1e100` for the max threshold,
    /// `0.95` for the decay factor and `0.0` for the initial VSIDS value).
    ///
    ...
    pub fn new(backup_brancher: BackupSelector) -> Self {
        CpVsids {
            ...
            interval_tracker: HashMap::default(),
    ...
    fn resize_heap(&mut self, id: (DomainId, PredicateType)) {
        while self.heap.len() <= id.index() {
            let next_key: (DomainId, PredicateType) = StorageKey::create_from_index(self.heap.len());
            self.heap.grow(next_key, DEFAULT_VSIDS_VALUE);
        }
    }
    ...
    fn bump_activity(&mut self, predicate: Predicate) {
        ...
        let id = (predicate.get_domain(), predicate.get_predicate_type());
        ...
        if activity + self.increment >= self.max_threshold {
            ...
            for (_, hashmap)in self.interval_tracker.iter_mut() {
                let _ = hashmap.values_mut().map(|v| *v / self.max_threshold);
            }
        ...
        // Additional bumping for the interval tracker
        if self.interval_tracker.contains_key(&(predicate.get_domain(), predicate.get_predicate_type())) {
            let btree = self.interval_tracker.get_mut(&(predicate.get_domain(), predicate.get_predicate_type())).unwrap();
            let _ = btree.entry(predicate.get_right_hand_side()).and_modify(|counter| *counter += self.increment).or_insert(self.increment);
        } else {
            let _ = self.interval_tracker.insert((predicate.get_domain(), predicate.get_predicate_type()), HashMap::default());
            if let Some(btree) = self.interval_tracker.get_mut(&(predicate.get_domain(), predicate.get_predicate_type())) {
                let _ = btree.entry(predicate.get_right_hand_side()).and_modify(|counter| *counter += self.increment).or_insert(self.increment);
            } else {
                unreachable!()
            }
        ...
        match predicate.get_predicate_type() {
            PredicateType::Equal => {
                if let Some(max) = self.max_eq {
                    if predicate.value().eq(&max) {
                        // Keep the maximum counter value in the heap
                        self.heap.increment(id, self.increment);
                    } else {
                        // Check what the max currently is and if it should be changed
                        let mut max_counter: Option<f64> = None;
                        if self.interval_tracker.contains_key(&(predicate.get_domain(), predicate.get_predicate_type())) {
                            let hashmap = self.interval_tracker.get_mut(&(predicate.get_domain(), predicate.get_predicate_type())).unwrap();
                            for (_, counter) in hashmap {
                                if max_counter.is_none() {
                                    max_counter = Some(*counter);
                                } else if max_counter.unwrap() < *counter {
                                    max_counter = Some(*counter);
                                }
                            }
                        }
                        // At this point, the maximum is stored in the max counter, and we need to update the heap
                        if let Some(counter) = max_counter {
                            self.heap.set_value((predicate.get_domain(), predicate.get_predicate_type()), counter);
                        }
                    }
                } else {
                    // If it is none, instantiate current value as max and increment the heap
                    self.max_eq = Some(predicate.value());
                    self.heap.increment(id, self.increment);
                }
            },
            PredicateType::NotEqual => {
                if let Some(max) = self.max_ne {
                    if predicate.value().eq(&max) {
                        // Keep the maximum counter value in the heap
                        self.heap.increment(id, self.increment);
                    } else {
                    ...
                    // If it is none, instantiate current value as max and increment the heap
                    self.max_ne = Some(predicate.value());
                    self.heap.increment(id, self.increment);
                }
            }
            _ => self.heap.increment(id, self.increment)
        }
    ...
    fn next_candidate_predicate(&mut self, context: &mut SelectionContext) -> Option<Predicate> {
        ...
        loop {
            ...
            if let Some((candidate_dp, max)) = self.heap.peek_max() {

                let d_id = candidate_dp.0;
                let pt = candidate_dp.1;
                // Get the exact predicate we want
                let btree: &mut HashMap<i32,f64>;
                if let Some(_btree) = self.interval_tracker.get_mut(candidate_dp) {
                    btree = _btree;
                } else {
                    let _ =self.interval_tracker.insert((d_id, pt), Default::default());
                    btree =  self.interval_tracker.get_mut(candidate_dp).unwrap_or_else(|| unreachable!());
                }
                let lb = assignments.get_initial_lower_bound(d_id);
                let ub = assignments.get_initial_upper_bound(d_id);

                let mut same_dec:bool = false;
                let mut point_dec:bool = false;
                let mut extreme: bool = false;
                let mut gvsids_val: i32 = 0;
                let mut candidate: Option<Predicate> = None;
                match pt {
                    PredicateType::LowerBound => {
                        let _value = btree.iter()
                            .max_by(|a, b| a.1.total_cmp(b.1))
                            .map(|(&key, _)| key);
                        let smaller_ub = ub -  (EXTREME_PERCENTILE * (ub-lb) as f64) as i32;
                        if let Some(vsids_val) = _value {
                            for i in (lb..smaller_ub).rev() {
                                if let Some(value) = btree.get(&i) {
                                    // If we encounter the first predicate which has a value, we know it
                                    // is the one that applies to the largest amount of premises
                                    // For lower bound, the highest value in the domain that was
                                    // observed in a conflict is the lowest assignment which applies to
                                    // all premises
                                    if value > &0.0 {
                                        candidate = Some(d_id.atomic_greater_than(i));
                                        if i == vsids_val {
                                            same_dec = true;
                                        } else {
                                            gvsids_val = vsids_val;
                                            // self.statistics.average_difference_vsids_cca_value
                                            //     .add_term(abs(i - vsids_val) as usize);
                                        }
                                        break;
                                    }
                                }
                            }
                            // As it was not found in the non-extreme part of the domain,
                            // we continue the linear search over the domain, from the most pruning
                            // value
                            for i in (smaller_ub..ub) {
                                if let Some(value) = btree.get(&i) {
                                    if value > &0.0 {
                                        candidate = Some(d_id.atomic_greater_than(i));
                                        if i == vsids_val {
                                        ...
                                        extreme = true;
                                        // self.statistics.extreme_points += 1;
                                        break;
                                    }
                                }
                            }
                    ...
                    PredicateType::UpperBound => {
                        ...
                        let larger_lb = lb + (EXTREME_PERCENTILE * (ub-lb) as f64) as i32;
                        if let Some(vsids_val) = _value {
                            for i in larger_lb..ub {
                                if let Some(value) = btree.get(&i) {
                                    // If we encounter the first predicate which has a value, we know it
                                    // is the one that applies to the largest amount of premises
                                    // For upper bound, the lowest value in the domain that was
                                    // observed in a conflict is the highest assignment which applies to
                                    // all premises
                                    if value > &0.0 {
                                        candidate = Some(d_id.atomic_less_than(i));
                                        if i == vsids_val {
                                            // self.statistics.vsids_aca_same_decision +=1
                                            same_dec = true;
                                        } else {
                                            gvsids_val = vsids_val;
                                            // self.statistics.average_difference_vsids_cca_value
                    ...
                    PredicateType::NotEqual => {
                        ...
                        for i in lb..ub {
                            if let Some(value) = btree.get(&i) {
                                if value == max {
                                    candidate = Some(d_id.atomic_not_equal(i));
                                    break;
                                }
                            }
                        }
                    ...
                    PredicateType::Equal => {
                        ...
                        for i in lb..ub {
                            if let Some(value) = btree.get(&i) {
                                if value == max {
                                    candidate = Some(d_id.atomic_equal(i));
                                    break;
                                }
                            }
                        }
                ...
                if let Some(predicate) = candidate {
                    if context.is_predicate_assigned(predicate) {
                        self.statistics.num_assigned_predicates_encountered += 1usize;
                        if let Some(domain) = self.heap.pop_max() {
                            let predicate_id = self.predicate_id_info.get_id(predicate);
                            self.heap.delete_key(domain);
                        ...
                        // If predicate is chosen, keep track of statistics
                        if same_dec {
                            self.statistics.vsids_cca_same_decision += 1usize;
                        } else {
                            self.statistics.average_difference_vsids_cca_value
                                .add_term(abs(predicate.get_right_hand_side() - gvsids_val) as usize);
                        }
                        self.statistics.cca_decisions += 1;
                        self.statistics.extreme_points += extreme as usize;
                        self.statistics.point_decisions += point_dec as usize;
...
impl<BackupBrancher: Brancher> Brancher for CpVsids<BackupBrancher> {
...
// #[cfg(test)]
// mod tests {
//     use super::{AutonomousSearch, CpVsids};
...
//
//         let mut brancher = CpVsids::default_over_all_variables(&assignments);
